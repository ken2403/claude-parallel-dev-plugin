...
`git worktree list` regardless of location (sa/ha/ca and any
...
`$ARGUMENTS`, clean **every** merged worktree in the whole repo — sa, ha, ca,
and any other, regardless of location. Do **not** narrow to
`.claude/worktrees/sa/` just because this is the sa command; a
...
`bash "$CLAUDE_SKILL_SA_DIR/scripts/merge-check.sh"`), or there is no PR and the
