...
# secret/credential file. sa runs simple features in worktrees; this is a
# cheap last line of defense against rewriting a .env or a key.
#
...
if [ -n "${blocked:-}" ]; then
  echo "sa: refusing to edit sensitive file '$file_path'. If this is intentional, edit it manually or remove it from the guard list." >&2
  exit 2
fi
exit 0
