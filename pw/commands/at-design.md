...
echo "Base branch: $(_PD=""; for _d in "${CLAUDE_PLUGIN_ROOT:-}" ./pw ../pw ../../pw; do [ -d "$_d/scripts" ] && _PD="$_d" && break; done 2>/dev/null; [ -n "${PW_PLUGIN_DIR:-}" ] && _PD="$PW_PLUGIN_DIR"; "$_PD/scripts/detect-base-branch.sh" 2>/dev/null || echo "main")"
ls -la | head -15
git log --oneline -5
```
