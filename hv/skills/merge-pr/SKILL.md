---
name: merge-pr
description: Merge a reviewed PR (hv-managed or standalone) after confirming it is genuinely ready — approved, green CI, up to date with base, no unresolved blocking feedback. Use once /hv:review-pr approves a feature's PR. Refuses to merge on red checks or missing review.
...
- `reviewDecision` is not APPROVED (a hv PR should have a `/hv:review-pr` pass).
...
- `mergeable` is false / `mergeStateStatus` indicates conflicts or out-of-date —
  ...
  `/hv:apply-feedback` pass can help) and re-run this skill.
...
worktree can be reclaimed with **`/hv:clean-agents`**, and any features that
