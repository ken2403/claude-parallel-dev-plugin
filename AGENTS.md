...
- `sa/agents/<name>.md` — subagents (`implementer` **sonnet**·effort medium, `verifier` **opus**·effort high). No `janitor`/`analyzer` — sa stays light.
...
- **Model/effort**: hybrid Sonnet build / Opus review, graded — implementation & feedback on the latest **Sonnet** (`simple-implement` medium, `apply-feedback` medium, `implementer` medium); the adversarial review/verify path on **Opus** (`review-pr` high, `resolve-conflicts` high, `verifier` high); `merge-pr` + `clean-worktrees` **haiku**·effort low (mechanical guardrails — merge-pr's preflight is field checks on `gh pr view` JSON with `gh`/branch-protection refusing ineligible merges server-side); `code-review` omits both (standards skill). Models are pinned via the `sonnet`/`opus`/`haiku` aliases (not IDs) so they track the latest — `sa` is allowed to pin (unlike model-agnostic `ha`).
