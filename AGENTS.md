...
- **Model/effort**: all-Sonnet cross-checks with targeted Opus escalation, graded — the thesis is **error-rate multiplication**: several cheap, independent checks (mandatory red-green, a risk-scaled pre-PR `verifier` pass in `simple-implement`, three mutually blind `verifier` lenses in `review-pr`) miss less together than one expensive correlated pass. Build & feedback on the latest **Sonnet** (`simple-implement` medium, `apply-feedback` medium, `implementer` medium); review/verify also **Sonnet** (`review-pr` high, `verifier` high) with **deterministic escalation** to `deep-verifier` (**opus**·high; triggers: risky surface / UNCERTAIN on a would-be-blocking claim / conflicting verifier verdicts — it gets only the unresolved claim, never a re-review); `resolve-conflicts` stays **opus**·high (rare, judgment-dense, silent-corruption risk; its integration check dispatches `deep-verifier`); `merge-pr` + `clean-worktrees` **haiku**·effort low (mechanical guardrails — merge-pr's preflight is field checks on `gh pr view` JSON with `gh`/branch-protection refusing ineligible merges server-side); `code-review` omits both (standards skill; owns the blocking rule "behavior change without a covering test"). Models are pinned via the `sonnet`/`opus`/`haiku` aliases (not IDs) so they track the latest — `sa` is allowed to pin (unlike model-agnostic `ha`).
