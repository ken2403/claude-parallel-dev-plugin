# Design: `hv` — Opus 4.8-native parallel autonomous feature-dev plugin
...
- `SessionStart` — (optional) note hv context.
...
- New plugin **co-located** in `hv/` subdirectory (pw stays at repo root, untouched).
- `hv/.claude-plugin/plugin.json` (name `hv`, version `0.1.0`).
- Root `.claude-plugin/marketplace.json` gains a second entry `{name:"hv", source:"./hv"}`.
- Self-contained scripts under `hv/scripts/` (no `../` references — cache-safe).
...
hv/
  ...
  README.md
docs/specs/2026-05-28-hv-plugin-design.md   (this file)
```
