...
#      @@IMPLEMENT_COMMAND@@ created under .claude/worktrees/@@PLUGIN@@/<slug>), REUSE it.
#   2. If <branch> is checked out in the MAIN working copy, refuse (exit 1) —
#      this skill must not edit the user's main checkout.
#   3. Otherwise create a fresh worktree under .claude/worktrees/@@PLUGIN@@/<slug>,
...
if [ -n "$existing_path" ]; then
  if [ "$existing_is_main" = 1 ]; then
    ...
    echo "  @@ATTACH_MAIN_REFUSAL_SUBJECT@@ will not edit the main checkout. Switch it off the branch" >&2
...
# --- 2. None exists — create one under .claude/worktrees/@@PLUGIN@@/<slug> -------------
SLUG="$(printf '%s' "$BRANCH" | tr '/' '-' | tr -cs 'A-Za-z0-9._-' '-')"
WT_DIR="$ROOT/.claude/worktrees/@@PLUGIN@@/$SLUG"
[ -e "$WT_DIR" ] && {
...
mkdir -p "$ROOT/.claude/worktrees/@@PLUGIN@@"
