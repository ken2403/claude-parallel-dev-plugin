...
```
parent-directory/
├── .claude-parallel-dev-plugin/  # このプラグイン
...
```bash
cd your-project
claude --plugin-dir ../.claude-parallel-dev-plugin
```
...
{
  "plugins": [
    "/path/to/.claude-parallel-dev-plugin"
  ]
}
```
...
```bash
cp ../.claude-parallel-dev-plugin/examples/CLAUDE.project-template.md ./CLAUDE.md
# プロジェクトに合わせて編集
```
...
```bash
mkdir -p .claude
cp ../.claude-parallel-dev-plugin/examples/hooks-python.json .claude/settings.json
```
