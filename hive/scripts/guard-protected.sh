#!/usr/bin/env bash
# ==============================================================================
# PreToolUse(Edit|Write) guard: block edits to sensitive files.
#
...
# secret/credential file. Autonomous workers run unattended, so this is a
# cheap last line of defense against an agent rewriting a .env or a key.
#
...
if [ -n "${blocked:-}" ]; then
  echo "hive: refusing to edit sensitive file '$file_path'. If this is intentional, edit it manually or remove it from the guard list." >&2
  exit 2
fi
exit 0
