...
`hive` designs and decomposes work, dispatches **one isolated background agent per
...
control). `pw` is untouched; `hive` is installed alongside it.
...
hive maps each of those to a native platform capability:

| Goal | How hive does it |
|---|---|
| Many features in parallel | one `claude --bg` agent per feature, dispatched by `/hive:launch` |
...
| Autonomous → PR | `/hive:worker` runs understand → implement → verify → PR by itself |
| Right-sized PRs | `/hive:design` sizes each feature by **risk × independence** |
| Safe & consistent | auto-applied `security-review` + `codebase-consistency` skills; a hook blocks edits to secrets |
| Better designs, not just patches | `/hive:design` may propose a superior architecture; deviations are explicit, never silent |
...
```
/hive:design  →  /hive:launch  →  (agents run /hive:worker in parallel)  →  /hive:status
                                                                              │
                                            /hive:review → /hive:fix → /hive:merge → /hive:cleanup
```

1. **`/hive:design <#issue | "spec" | @file>`** — explores the codebase, writes a
...
2. **`/hive:launch <manifest>`** — validates the manifest (no shared files, no
   dependency cycles), then dispatches one background agent per feature in
   dependency waves. Each runs `/hive:worker`.
3. **`/hive:worker <feature>`** *(runs inside each agent)* — lands on the feature
...
4. **`/hive:status`** — one table of every agent + its PR, with triage suggestions.
5. **`/hive:review <pr> [--comment]`** — independent adversarial review.
6. **`/hive:fix <pr>`** — applies review feedback (parallel for 3+ files), updates PR.
7. **`/hive:merge <pr>`** — merges only when approved + green + mergeable.
8. **`/hive:cleanup`** — stops finished agents, prunes merged worktrees/branches
...
**Skills** (`/hive:*`): `design`, `launch`, `worker`, `status`, `review`, `fix`,
...
- `git` (worktrees), `gh` (PRs), and ideally `jq` (richer `/hive:status`).
- **Permissions for hands-off runs:** background agents auto-accept edits with
  ...
  `--dangerously-skip-permissions`. `/hive:launch` surfaces this.
...
`hive` is the modern successor to `pw`. `pw` orchestrates via tmux + shell
scripts; `hive` uses native background agents and the `Agent` tool. Both ship from
this marketplace; pick `hive` for new work.
