---
name: launch
description: Dispatch the hive — start one isolated background agent per feature from a /hive:design manifest, each running /hive:worker to drive its feature to a PR. Use after design when you're ready to run features in parallel. Respects dependencies, so independent features start now and dependent ones wait. Each agent auto-isolates in its own git worktree, so features never interfere.
...
feature becomes one `claude --bg` session that runs `/hive:worker`; Claude Code
...
- Existing hive agents: !`claude agents --json 2>/dev/null | { command -v jq >/dev/null 2>&1 && jq -r '[.[]?|select((.name//"")|startswith("hive/"))]|length' || cat; } 2>/dev/null || echo 0`

## Step 1 — Load and validate the manifest

Parse the manifest (from `@file`, pasted JSON, or the previous `/hive:design`
...
to a spec file and pass its path, which is shell-safe. Use a `hive/<id>` name so
`/hive:status` and `/hive:cleanup` can find these agents:

```bash
# FEATURE_JSON = the single feature object, compact JSON
SPEC_DIR="$(git rev-parse --show-toplevel)/.hive/specs"
mkdir -p "$SPEC_DIR"
...
claude --bg \
  ...
  "/hive:worker $SPEC_FILE"
```

The prompt is now just `/hive:worker <path>` — no special characters — and the
...
- Only launch the next wave once its prerequisites have **open/merged PRs** (check
  with `/hive:status`); don't start a dependent feature against unlanded work.
...
Then tell the user:
- Watch progress with **`/hive:status`** (or the native `claude agents` view).
- Review finished PRs with **`/hive:review <pr>`**, fix feedback with
  **`/hive:fix`**, merge with **`/hive:merge`**, and reclaim worktrees/agents with
  **`/hive:cleanup`** once merged.

result: launched <n> hive agents (<wave summary>).
