---
name: worker
description: Autonomously implements ONE feature end-to-end — understand, implement, adversarially verify, and open a right-sized PR — inside an isolated worktree. This runs as the main loop of a hive background agent; /hive:launch dispatches it. Invoke directly only when you want to drive a single feature to a PR autonomously in the current session.
argument-hint: <feature spec — JSON from /hive:design, or a plain-text task>
model: opus
effort: xhigh
...
isolated worktree (a hive background agent moved here before its first edit, or
...
The assignment may be (a) a **path to a JSON spec file** (how `/hive:launch`
...
skip this and do not soften it — it is the reason a hive feature can be trusted
...
`/hive:design`). If it grew well past budget, that is a signal the feature
