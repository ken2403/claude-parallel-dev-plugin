---
name: merge
description: Merge a reviewed hive (or any) PR after confirming it is genuinely ready — approved, green CI, up to date with base, no unresolved blocking feedback. Use once /hive:review approves a feature's PR. Refuses to merge on red checks or missing review.
...
- `reviewDecision` is not APPROVED (a hive PR should have a `/hive:review` pass).
...
- `mergeable` is false / `mergeStateStatus` indicates conflicts or out-of-date —
  ...
  `/hive:fix` pass can help) and re-run this skill.
...
worktree can be reclaimed with **`/hive:cleanup`**, and any features that
