---
name: review
description: Critically review a hive (or any) PR for correctness, security, and codebase consistency, using adversarial verification rather than a rubber stamp. Use to review a feature's PR before merging, or whenever you want a high-confidence review of a pull request. Posts inline comments with --comment.
...
A hive worker already self-verified before opening its PR. Your job here is an
...
Hand off: clean review → `/hive:merge <n>`; changes requested → `/hive:fix <n>`.
